...
const ArtistPage = () => {
    ...
    const fetchSingleArtist = async () => {
        ...
        try {
            const response = await fetch(
                `https://striveschool-api.herokuapp.com/api/deezer/search?q=${artistToShow.name}`,
                {
                    method: "GET",
                    headers: headers,
                }
        ...
        <div className="row">
            ...
            >
                {dataLoaded && (
                    <>
                        <button
                            className="btn btn-success mr-2 mainButton d-block"
                            id="playButton"
                        >
                            PLAY
                        </button>
                        <button
                            className="btn btn-outline-light mainButton d-block"
                            id="followButton"
                        >
                            FOLLOW
                        </button>
            ...
            <div className="col-12 col-md-10 offset-md-1 col-lg-10 offset-lg-1 p-0">
                ...
                {topTracks.map((track) => (
                    ...
                    <div className="d-flex flex-column">
                        <Link className="h5 text-light" to={"/album/" + track.album.id}>Track:&nbsp; 
                            {track.title}
                        </Link>
                        <Link
